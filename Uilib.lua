-- Made by samet & edited by Caualelek32 and Buzinildo
...
local Library do 
    ...
    local Themes = {
        ["Preset"] = {
            ["AccentGradient"] = FromRGB(150, 0, 0),     -- Red accent gradient
            ...
            ["Accent"] = FromRGB(255, 50, 50),           -- Red accent for consistency
    ...
    local Instances = { } do
        ...
        Instances.MakeDraggable = function(self)
            ...
            local Set = function(Input)
                ...
                local NewAbsoluteX = StartAbsolutePosition.X + DragDelta.X
                local NewAbsoluteY = StartAbsolutePosition.Y + DragDelta.Y

                NewAbsoluteX = MathClamp(NewAbsoluteX, ParentPosition.X, ParentPosition.X + ParentSize.X - GuiSize.X)
                NewAbsoluteY = MathClamp(NewAbsoluteY, ParentPosition.Y, ParentPosition.Y + ParentSize.Y - GuiSize.Y)

                local Anchor = Gui.AnchorPoint
                local PositionX = (NewAbsoluteX - ParentPosition.X) + (GuiSize.X * Anchor.X)
                local PositionY = (NewAbsoluteY - ParentPosition.Y) + (GuiSize.Y * Anchor.Y)

                Gui.Position = UDim2FromOffset(PositionX, PositionY)
            end
    ...
    Library.RefreshConfigsList = function(self, Element)
        local CurrentList = self.CurrentConfigList or { }
        local List = { }
    ...
    do 
        ...
        Library.KeybindList = function(self, Title)
            ...
            KeybindList.Instance = Items["KeybindsList"].Instance

            function KeybindList:SetVisibility(Bool)
                Items["KeybindsList"].Instance.Visible = Bool == true
            end
            ...
            function Window:SetOpen(Bool)
                ...
                if Window.IsOpen then 
                    ...
                    for _, Value in Library.OpenFrames do
                        if Value and Value.IsOpen and type(Value.SetOpen) == "function" then
                            TableInsert(FramesToClose, Value)
                        end
                    end

                    for _, Value in FramesToClose do
                        Value:SetOpen(false)
                    end
                end
        ...
        Library.Pages.GlobalChat = function(self, Side)
            ...
            function GlobalChat:SetVisibility(Bool)
                ...
                Items["GlobalChat"].Instance.Parent = Bool and Page.ColumnsData[Side].Instance or Library.UnusedHolder.Instance
            end
        ...
        Library.Sections.Dropdown = function(self, Data)
            ...
            function Dropdown:SetOpen(Bool)
                ...
                if Dropdown.IsOpen then 
                    ...
                    for Index, Value in Library.OpenFrames do 
                        if Value ~= Dropdown and not Dropdown.Section.IsSettings and type(Value.SetOpen) == "function" then 
                            Value:SetOpen(false)
                        end
                    end
            ...
            function Dropdown:Refresh(List)
                ...
                for _, Value in Dropdown.Options do
                    TableInsert(NamesToRemove, Value.Name)
                end

                for _, Name in NamesToRemove do
    ...
    Library.CreateSettingsPage = function(self, Window, KeybindList)
        ...
        local ConfigsSection = Page:Section({Name = "Configs", Side = 2}) do 
            ...
            local NormalizeConfigName = function(Name)
                if not Name then
                    return nil
                end
                ...
                if not StringFind(Name:lower(), "%.json$") then
                    Name = Name .. ".json"
                end
            ...
            local ResolveConfigPath = function(Name)
                local Normalized = NormalizeConfigName(Name)
                if not Normalized then
                    return nil, nil
                end

                return Library.Folders.Configs .. "/" .. Normalized, Normalized
            end
            ...
            ConfigsSection:Button({
                Name = "Create",
                Callback = function()
                    local Path, FileName = ResolveConfigPath(Library.Flags.ConfigsName or ConfigName)
                    if Path and FileName then
                        if not isfile(Path) then
                            writefile(Path, Library:GetConfig())
                            ConfigSelected = FileName
            ...
            ConfigsSection:Button({
                Name = "Delete",
                Callback = function()
                    local _, FileName = ResolveConfigPath(ConfigSelected)
                    if FileName then
            ...
            ConfigsSection:Button({
                Name = "Load",
                Callback = function()
                    local Path = ResolveConfigPath(ConfigSelected)
